...
int main(int argc, char **argv)
{
	...
	{
		ErrorHandling("hClntSocket(), error");
	}
	else
	...
	if (connect(hClntSock, (SOCKADDR*)&servAddr, sizeof(servAddr)) == SOCKET_ERROR)
	{
		...
		puts("Connecting with Server...Success");
...
void enterName(void)
{
	...
	send(hClntSock, clntName, sizeof(clntName), 0);
	// receive name of the client
	gotoxy(20, 15);
	puts("Waiting for other player....");
	strLen = recv(hClntSock, servName, sizeof(servName) - 1, 0);
	...
	while (1)
	{
		...
		clntScore = p1diceNum;
		send(hClntSock, (char*)&clntScore, sizeof(clntScore), 0);

		gotoxy(20, 15);
		puts("Waiting for other player....");

		recv(hClntSock, (char*)&servScore, sizeof(servScore), 0);
		...
		if (clntScore > servScore)
		{
			printf("\n\n\n\n                               Nice~ %s님의 선공입니다!!", clntName);
			first = 1;
			break;
		}
		else if (servScore > clntScore)
		{
			printf("\n\n\n\n                                %s님의 선공입니다 ㅠㅠ", servName);
			first = 2;
			break;
		}
